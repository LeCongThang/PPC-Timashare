                ...
                <div class="col-md-12">
                    <div class="box box-primary">
                        ...
                        <div class="box-header with-border text-center">
                            <div class="pull-left">
                                <a href="<?= BASE_URL_ADMIN ?>controllerthongbao/create"
                                   class="btn btn-success"><i
                                        class="glyphicon glyphicon-th-large"></i>&nbsp <b>Thêm thông báo</b></a>
                        ...
                        <div class="box-body">
                            <table class="table">
                                ...
                                <?php foreach ($list_deals as $key => $deals): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="<?= BASE_URL_ADMIN ?>controllerthongbao/update/<?= $deals['id'] ?>"
                            ...
                            <div id="page" class="pages col-md-12" style="text-align: center">
                                <p>Trang
                                    <?php
                                    if ($pageList != 1)
                                        echo '<a href="' . BASE_DIR_ADMIN . 'controllerthongbao/index/' . ($pageEnd - 5) . '">&lsaquo;&lsaquo;  </a>';
                                    for ($i = $pageEnd - 4; $i <= $lastPage; $i++) {
                                        if ($i == $currentPage)
                                            echo '<a style="margin-right: 3px;font-weight: bolder;color:black" class="active"  href="' . BASE_DIR_ADMIN . 'controllerthongbao/index/' . ($i) . '">' . $i . '</a>';
                                        else if ($i != 0)
                                            echo '<a style="margin-right: 3px" href="' . BASE_DIR_ADMIN . 'controllerthongbao/index/' . ($i) . '">' . $i . '</a>';
                                    }
                                    if ($pageListLasted != $pageList)
                                        echo '<a href="' . BASE_DIR_ADMIN . 'controllerthongbao/index/' . ($pageEnd + 1) . '">  &rsaquo;&rsaquo;</a>';
                                    ?>
                                </p>
                            </div>
                        </div>
...
<script>
    function ftDelete($id) {
        ...
        if(answer){
            $.ajax({
                type: "POST",
                url: "<?=BASE_URL_ADMIN?>controllerthongbao/delete/"+$id,
                success: function(data){
                    window.location.href='<?=BASE_URL_ADMIN?>controllerthongbao/index';
